...
using BoardMaui.Extensions.TimeZone;
using System.Text;
...
public class GeoLocation : IGeoLocation
{
    ...
    private readonly int FINAL_BEARING = 2;
    private readonly int INITIAL_BEARING = 1;
    ...
    {
        get => timeZone; set => timeZone = value;
    }
    ...
    /// </returns>
    public virtual long GetLocalMeanTimeOffset(DateTime date) => (long)((Longitude * 4 * MINUTE_MILLIS) - TimeZone.UtcOffset(date));
    ...
    public virtual double GetGeodesicInitialBearing(GeoLocation location) => VincentyFormula(location, INITIAL_BEARING);
    ...
    ///<param name = "location">
    ///  the destination location </param>
    public virtual double GetGeodesicFinalBearing(GeoLocation location) => VincentyFormula(location, FINAL_BEARING);
    ...
    ///<param name = "location">
    ///  the destination location </param>
    public virtual double GetGeodesicDistance(GeoLocation location) => VincentyFormula(location, DISTANCE);
    ...
    private double VincentyFormula(GeoLocation location, int formula)
    {
        ...
        double L = (location.Longitude - Longitude).ToRadians();
        double U1 = Math.Atan((1 - f) * Math.Tan(Latitude.ToRadians()));
        double U2 = Math.Atan((1 - f) * Math.Tan(location.Latitude.ToRadians()));
        ...
        // initial bearing
        double fwdAz = Math.Atan2(cosU2 * sinLambda, (cosU1 * sinU2) - (sinU1 * cosU2 * cosLambda)).ToDegree();
        // final bearing
        double revAz = Math.Atan2(cosU1 * sinLambda, (-sinU1 * cosU2) + (cosU1 * sinU2 * cosLambda)).ToDegree();
        return formula == DISTANCE ? distance : formula == INITIAL_BEARING ? fwdAz : formula == FINAL_BEARING ? revAz : double.NaN;
    }
    ...
    public virtual double GetRhumbLineBearing(GeoLocation location)
    {
        ...
        double dPhi =
            Math.Log(Math.Tan((location.Latitude.ToRadians() / 2) + (Math.PI / 4)) /
                            Math.Tan((Latitude.ToRadians() / 2) + (Math.PI / 4)));
    ...
    public virtual double GetRhumbLineDistance(GeoLocation location)
    {
        ...
        double dLon = Math.Abs(location.Longitude - Longitude).ToRadians();
        double dPhi =
            Math.Log(Math.Tan((location.Longitude.ToRadians() / 2) + (Math.PI / 4)) /
                            Math.Tan((Latitude.ToRadians() / 2) + (Math.PI / 4)));
        double q = Math.Abs(dLat) > 1e-10 ? dLat / dPhi : Math.Cos(Latitude.ToRadians());
    ...
    public virtual string ToXml()
    {
        ...
        _ = sb.Append("<GeoLocation>\n");
        _ = sb.Append("\t<LocationName>").Append(LocationName).Append("</LocationName>\n");
        _ = sb.Append("\t<Latitude>").Append(Latitude).Append("°").Append("</Latitude>\n");
        _ = sb.Append("\t<Longitude>").Append(Longitude).Append("°").Append("</Longitude>\n");
        _ = sb.Append("\t<Elevation>").Append(Elevation).Append(" Meters").Append("</Elevation>\n");
        _ = sb.Append("\t<TimezoneName>").Append(TimeZone.GetId()).Append("</TimezoneName>\n");
        _ = sb.Append("\t<TimeZoneDisplayName>").Append(TimeZone.GetDisplayName()).Append(
            "</TimeZoneDisplayName>\n");
        /*
