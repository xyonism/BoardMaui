...
public class GeoLocationUtils
{
    ...
    private static readonly int INITIAL_BEARING = 1;
    private static readonly int FINAL_BEARING = 2;
    ...
    /// <returns></returns>
    public static double GetGeodesicInitialBearing(GeoLocation location, GeoLocation destination) => VincentyFormula(location, destination, INITIAL_BEARING);
    ...
    /// <param name="destination">The destination.</param>
    /// <returns></returns>
    public static double GetGeodesicFinalBearing(GeoLocation location, GeoLocation destination) => VincentyFormula(location, destination, FINAL_BEARING);
    ...
    /// <param name="destination">The destination.</param>
    /// <returns></returns>
    public static double GetGeodesicDistance(GeoLocation location, GeoLocation destination) => VincentyFormula(location, destination, DISTANCE);
    ...
    private static double VincentyFormula(GeoLocation location, GeoLocation destination, int formula)
    {
        ...
        double L = (destination.Longitude - location.Longitude).ToRadians();
        double U1 = Math.Atan((1 - f) * Math.Tan(location.Latitude.ToRadians()));
        double U2 = Math.Atan((1 - f) * Math.Tan(destination.Latitude.ToRadians()));
    ...
    {
        double dLon = (destination.Longitude - location.Longitude).ToRadians();
        double dPhi =
            Math.Log(Math.Tan((destination.Latitude.ToRadians() / 2) + (Math.PI / 4)) /
                            Math.Tan((location.Latitude.ToRadians() / 2) + (Math.PI / 4)));
    ...
    public static double GetRhumbLineDistance(GeoLocation location, GeoLocation destination)
    {
        ...
        double dLat = (destination.Latitude - location.Latitude).ToRadians();
        double dLon = Math.Abs(destination.Longitude - location.Longitude).ToRadians();
        double dPhi =
            Math.Log(Math.Tan((destination.Longitude.ToRadians() / 2) + (Math.PI / 4)) /
                            Math.Tan((location.Latitude.ToRadians() / 2) + (Math.PI / 4)));
        double q = Math.Abs(dLat) > 1e-10
                       ? dLat / dPhi
                       : Math.Cos(location.Latitude.ToRadians());
