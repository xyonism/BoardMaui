...
using static Zmanim.JewishCalendar.JewishCalendar;

namespace BoardMaui.Extensions.JewishCalendar
{
    /// <summary>
    ...
    public class HebrewDateFormatter
    {
        ...
        private static readonly Zmanim.JewishCalendar.JewishCalendar jewishCalendar = new();
        ...
        {
            get => longWeekFormat; set => longWeekFormat = value;
        }
        ...
        {
            get => transliteratedShabbosDayOfweek; set => transliteratedShabbosDayOfweek = value;
        }
        ...
        {
            get => transliteratedHolidays; set => transliteratedHolidays = value;
        }
        ...
        {
            JewishHoliday holiday = jewishCalendar.GetJewishHoliday(dt, inIsrael);

            int index = (int)holiday;

            if (holiday == JewishHoliday.חנוכה)
            {
        ...
        {
            get => hebrewFormat; set => hebrewFormat = value;
        }
        ...
        {
            get => hebrewOmerPrefix; set => hebrewOmerPrefix = value;
        }
        ...
        {
            get => transliteratedMonths; set => transliteratedMonths = value;
        }
        ...
        {
            get => transliteratedParshios; set => transliteratedParshios = value;
        }
        ...
        public virtual string FormatDayOfWeek(DateTime dt)
        {
            ...
            if (hebrewFormat)
            {
                ...
                _ = sb.Append(longWeekFormat ? hebrewDaysOfWeek[dayOfWeek - 1] : FormatHebrewNumber(dayOfWeek));
                return sb.ToString();
            }
            else
            {
        ...
        {
            get => useGershGershayim; set => useGershGershayim = value;
        }
        ...
        {
            get => useLonghebrewYears; set => useLonghebrewYears = value;
        }
        ...
        {
            return HebrewFormat
                ? FormatHebrewNumber(jewishCalendar.GetDayOfMonth(dt)) + " " + FormatMonth(dt) + " " + FormatHebrewNumber(jewishCalendar.GetYear(dt))
                : jewishCalendar.GetDayOfMonth(dt) + " " + FormatMonth(dt) + ", " + jewishCalendar.GetYear(dt);
        }
        ...
        public virtual string FormatMonth(DateTime dt)
        {
            ...
            JewishMonth month = jewishCalendar.GetJewishMonth(dt);
            bool isLeapYear = jewishCalendar.IsLeapYearFromDateTime(dt);

            if (HebrewFormat)
            {
                if (isLeapYear && month == JewishMonth.ADAR)
                {
                    return hebrewMonths[13] + (useGershGershayim ? GERESH : ""); // return Adar I, not Adar in a leap year
                }
                else
                {
                    return isLeapYear && month == JewishMonth.ADAR_II
                    ? hebrewMonths[12] + (useGershGershayim ? GERESH : "")
                    : hebrewMonths[(int)month - 1];
                }
            }
            else
                if (isLeapYear && month == JewishMonth.ADAR)
        ...
        {
            DateTime dt = jewishCalendar.GetJewishDateTime(jewishYear, JewishMonth.TISHREI, 1);// set date to Rosh Hashana

            JewishYearType yearType = jewishCalendar.GetJewishYearType(dt);
            int roshHashanaDayOfweek = jewishCalendar.GetJewishDayOfWeek(dt);
            string returnValue = FormatHebrewNumber(roshHashanaDayOfweek);
            returnValue += yearType == JewishYearType.CHASERIM ? "\u05D7" : yearType == JewishYearType.SHELAIMIM ? "\u05E9" : "\u05DB";
            dt = jewishCalendar.GetJewishDateTime(jewishYear, JewishMonth.NISSAN, 15); // set to Pesach of the given year
        ...
        {
            return hebrewFormat ? daf.Masechta + " " + FormatHebrewNumber(daf.Page) : daf.MasechtaTransliterated + " " + daf.Page;
        }
        ...
        public virtual string FormatHebrewNumber(int number)
        {
            if (number < 0)
            {
                throw new ArgumentException("negative numbers can't be formatted");
            }
            else if (number > 9999)
            {
                throw new ArgumentException("numbers > 9999 can't be formatted");
            }
            ...
            if (number % 1000 == 0) // in year is 5000, 4000 etc
            {
                ...
                _ = sb.Append(" ");
                _ = sb.Append(ALAFIM); // add # of thousands plus word thousand (overide alafim boolean)
                return sb.ToString();
            ...
            else if (useLonghebrewYears && number >= 1000)
            {
                ...
                if (UseGershGershayim)
                {
                    _ = sb.Append(GERESH); // append thousands quote
                }

                _ = sb.Append(" ");
            }
            number %= 1000; // Remove 1000s
            int hundreds = number / 100; // # of hundreds
            _ = sb.Append(jHundreds[hundreds]); // add hundreds to String
            number %= 100; // Remove 100s
            if (number == 15) // special case 15
            {
                _ = sb.Append(tavTaz[0]);
            }
            else if (number == 16)
            {
                _ = sb.Append(tavTaz[1]);
            }
            else
            {
                int tens = number / 10;
                if (number % 10 == 0) // if evenly divisable by 10
                {
                    ...
                    {
                        _ = sb.Append(jTenEnds[tens]); // end letters so years like 5750 will end with an end nun
                    }
                    else
                    {
                        _ = sb.Append(jTens[tens]); // standard letters so years like 5050 will end with a regular nun
                    }
                }
                else
                {
                    _ = sb.Append(jTens[tens]);
                    number %= 10;
                    _ = sb.Append(jOnes[number]);
                }
            }
            if (UseGershGershayim)
            {
                ...
                {
                    _ = sb.Append(GERESH); // append single quote
                }
                else
                {
                    _ = sb.Insert(sb.Length - 1, GERSHAYIM);
                }
            }
