...
namespace BoardMaui.Extensions.JewishCalendar
{
    ...
    public class YomiCalculator
    {
        ...
        private static readonly DateTime dafYomiStartDate = new(1923, 9, 11);
        private static readonly int dafYomiJulianStartDay = GetJulianDay(dafYomiStartDate);
        private static readonly DateTime shekalimChangeDate = new(1975, 6, 24);
        private static readonly int shekalimJulianChangeDay = GetJulianDay(shekalimChangeDate);
        ...
        public static Daf GetDafYomiBavli(DateTime date)
        {
            ...
            for (int j = 0; j < blattPerMasechta.Length; j++)
            {
                ...
                if (dafNo < total)
                {
                    int blatt = 1 + blattPerMasechta[j] - (total - dafNo);
