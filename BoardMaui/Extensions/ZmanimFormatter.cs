...
public class ZmanimFormatter
{
    ...
    public virtual string Format(double milliseconds) => Format((int)milliseconds);
    ...
    ///  The time in milliseconds. </param>
    ///<returns> String The formatted <c>String</c> </returns>
    public virtual string Format(int millis) => Format(new Time(millis));
    ...
    public virtual string Format(Time time)
    {
        ...
        _ = sb.Append(time.Hours.ToString(hourNF));
        _ = sb.Append(":");
        _ = sb.Append(time.Minutes.ToString(minuteSecondNF));
        if (useSeconds)
        {
            _ = sb.Append(":");
            _ = sb.Append(time.Seconds.ToString(minuteSecondNF));
        }

        if (useMillis)
        {
            _ = sb.Append(".");
            _ = sb.Append(time.Milliseconds.ToString(milliNF));
        }
    ...
    public virtual string FormatDate(DateTime date, IDateWithLocation dateWithLocation) => DateFormat == "yyyy-MM-dd'T'HH:mm" ? GetXSDate(date, dateWithLocation) : date.ToString(DateFormat);
    ...
    ///       Datatypes]</a>. The date/time format is basically CCYY-MM-DDThh:mm,
    ...
    {
        string xsdDateFormat = "yyyy-MM-dd'T'HH:mm";
        //        
        ...
        StringBuilder buff = new(date.ToString(xsdDateFormat));
        ...
        else
        {
            ...
            char posneg = hrs < 0 ? '-' : '+';
            _ = buff.Append(posneg + FormatDigits(hrs) + ':' + FormatDigits(min));
        }
    ...
    public virtual string FormatXSDDurationTime(long millis) => FormatXSDDurationTime(new Time(millis));
    ...
    public virtual string FormatXSDDurationTime(Time time)
    {
        ...
        if (time.Hours != 0 || time.Minutes != 0 || time.Seconds != 0 || time.Milliseconds != 0)
        {
            ...
            if (time.Hours != 0)
            {
                _ = duration.Append(time.Hours + "H");
            }

            if (time.Minutes != 0)
            {
                _ = duration.Append(time.Minutes + "M");
            }

            if (time.Seconds != 0 || time.Milliseconds != 0)
            {
                _ = duration.Append(time.Seconds + "." + time.Milliseconds.ToString(milliNF));
                _ = duration.Append("S");
            }

            if (duration.Length == 1) // zero seconds
            {
                _ = duration.Append("T0S");
            }
    ...
    {
        ZmanimFormatter formatter = new(XSD_DURATION_FORMAT, "yyyy-MM-dd'T'HH:mm");
        ...
        List<MethodInfo> theMethods = ac.GetType().GetMethods().ToList();
#else
        ...
        object? @value = null;
        IList<Zman> dateList = [];
        IList<Zman> durationList = [];
        IList<string> otherList = [];
        for (int i = 0; i < theMethods.Count; i++)
        {
            if (IncludeMethod(theMethods[i]))
            {
                tagName = theMethods[i].Name[3..];
                try
                {
    ...
    {
        IList<string> methodWhiteList = [];
        // methodWhiteList.add("getName");

        IList<string> methodBlackList = [];
        ...
        return method.Name.ToLower().StartsWith("get") && (method.ReturnType == typeof(DateTime)
