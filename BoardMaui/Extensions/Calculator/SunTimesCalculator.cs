...
namespace BoardMaui.Extensions.Calculator;
...
public class SunTimesCalculator : AstronomicalCalculator
{
    ...
    ///</summary>
    private static double SinDeg(double deg) => Math.Sin(deg * 2.0 * Math.PI / 360.0);

    ///<summary>
    ///  acos of an angle, result in degrees
    ///</summary>
    private static double AcosDeg(double x) => Math.Acos(x) * 360.0 / (2 * Math.PI);

    ///<summary>
    ///  * asin of an angle, result in degrees
    ///</summary>
    private static double AsinDeg(double x) => Math.Asin(x) * 360.0 / (2 * Math.PI);

    ///<summary>
    ///  tan of an angle in degrees
    ///</summary>
    private static double TanDeg(double deg) => Math.Tan(deg * 2.0 * Math.PI / 360.0);

    ///<summary>
    ///  cos of an angle in degrees
    ///</summary>
    private static double CosDeg(double deg) => Math.Cos(deg * 2.0 * Math.PI / 360.0);
    ...
    ///</summary>
    private static double GetHoursFromMeridian(double longitude) => longitude / DEG_PER_HOUR;
    ...
    {
        double sunriseSunsetVar = isSunrise ? 6.0 : 18.0;
    ...
    ///</summary>
    private static double GetMeanAnomaly(int dayOfYear, double longitude, bool isSunrise) => (0.9856 * GetApproxTimeDays(dayOfYear, GetHoursFromMeridian(longitude), isSunrise)) - 3.289;
    ...
    ///</summary>
    private static double GetLocalMeanTime(double localHour, double sunRightAscensionHours, double approxTimeDays) => localHour + sunRightAscensionHours - (0.06571 * approxTimeDays) - 6.622;
